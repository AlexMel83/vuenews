        ...
        <div class="card-body d-flex flex-column">
            <div class="d-flex container flex-column m-0 p-0">
                ...
                <button type="button" class="buttons" @click="readMore" data-bs-toggle="collapse" role="button"
                    :data-bs-target="'#t' + myid" :aria-controls="'t' + myid" aria-expanded="false">Read {{ moreless
                    }}</button>
            </div>
            <p class="collapse card-text" :id="'t' + myid">{{ article.content }}</p>
            <p class="card-text"><small class="text-body-secondary"> <a :href="article.url" target="_blank"
                        class="link"><br> Source: {{
                            article.source.name }}</a></small><small
